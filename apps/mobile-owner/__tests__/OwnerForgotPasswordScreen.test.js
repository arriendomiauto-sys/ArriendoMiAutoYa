...
import { OwnerForgotPasswordScreen } from "../src/owner/auth/OwnerForgotPasswordScreen";
...
describe("OwnerForgotPasswordScreen · recuperar contraseña", () => {
  beforeEach(() => {
    jest.useFakeTimers();
  ...
  it("invita a escribir el correo y ofrece volver a entrar", () => {
    const t = textOf(renderTree(<OwnerForgotPasswordScreen onNavigate={() => {}} />));
  ...
  it("sin correo lo avisa en la pantalla, sin ventana emergente ni llamar al servidor", async () => {
    const tr = renderTree(<OwnerForgotPasswordScreen onNavigate={() => {}} />);
  ...
  it("'Volver a entrar' lleva al login, tanto en el formulario como en la confirmación", async () => {
    ...
    const tr = renderTree(<OwnerForgotPasswordScreen onNavigate={onNavigate} />);
